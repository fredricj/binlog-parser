...
SRC_DIR := $(ROOT_DIR)/src/zalora/binlog-parser

TEST_DB_NAME := test_db
TEST_DB_SCHEMA_FILE := data/fixtures/test_db.sql

all:
	env CGO_ENABLED=0 $(GOCC) -C $(SRC_DIR) install -ldflags '-s' $(SRC_DIR)/...
...
unit-test:
	$(info ************ UNIT TESTS ************)
	env TZ="UTC" env DATA_DIR=$(CURDIR)/data $(GOCC) -C $(SRC_DIR) test -tags=unit -cover $(SRC_DIR)/...

integration-test: integration-test-setup
	$(info ************ INTEGRATION TESTS ************)
	env TEST_DB_DSN="root@/$(TEST_DB_NAME)" env TZ="UTC" env DATA_DIR=$(CURDIR)/data $(GOCC) -C $(SRC_DIR) test -tags=integration -cover $(SRC_DIR)/...
